...
class EnoTiledImgNav(AtaBase):
  ...
  def plasmaMoveCb(self, coords: list[int]): 
    try:
      if self.verbose: self.msg("plasmaMoveCb: " + str(coords))
      dx, dy = coords
      x,   y = self.eti.imgPos[0] + dx, self.eti.imgPos[1] + dy
      self.eti.imgPos = (x,y)
    except: self.err("plasmaMoveCb")

  def setPlasmaMoveDeposit(self, depositMapSimpleUpdate):
    self.depositMapSimpleUpdate = depositMapSimpleUpdate
  ...
  def on_key_down(self, key):
    ...
    cp  = self.cursorsPressed
  
    if self.shiftPressed:
      nudge = self.smallnudge
      esi   = self.eti.shiftImg
      if key is keys.LEFT:  esi(-nudge, 0); cp['L'] = True
      if key is keys.RIGHT: esi( nudge, 0); cp['R'] = True
      if key is keys.UP:    esi(0, -nudge); cp['U'] = True
      if key is keys.DOWN:  esi(0,  nudge); cp['D'] = True
    else:
      nudge = self.bignudge
      eai   = self.eti.animImg
      if key is keys.LEFT:  eai(-nudge, 0);  cp['L'] = True
      if key is keys.RIGHT: eai( nudge, 0);  cp['R'] = True
      if key is keys.UP:    eai(0, -nudge);  cp['U'] = True
      if key is keys.DOWN:  eai(0,  nudge);  cp['D'] = True
  ...
  def on_mouse_move(self, rel):
    try:
      if self.mouseDown: 
        ...
        if self.moveImmediateIgnorePlasma:
          x,  y  = self.eti.imgPos[0] + dx, self.eti.imgPos[1] + dy
          self.eti.imgPos = (x,y)
          if self.verbose: print(rel, self.eti.imgPos)
        else:
          try:
            dmsu = self.depositMapSimpleUpdate
            if dmsu is None: 
              self.msg("on_mouse_move: plasma movement stub uninitiated")
              return False
            dmsu([dx,dy]) 
          except: self.err("on_mouse_move plasma deposit")

    except: self.err("on_mouse_move")
