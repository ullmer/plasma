...
int64 SlawList::IndexOf (bslaw s, unt64 start) const
{
  ...
  // The lambda correctly captures 's' by reference.
  it = find_if(cmps.begin(), cmps.end(),
               [&s](const detail::SlawRef& ref) {
                 return ref.Equals(s);
               });
#else // Pre-C++11 (C++03/98) fallback
  // Keep the original bind2nd/mem_fun_ref for older standards.
  it = find_if(cmps.begin(), cmps.end(),
...
//                              bind2nd (mem_fun_ref (&SlawRef::Equals), s)); //problems with C++17
//                              &s {return ref.Equals(s); });
  if (it == elements_.end ())
    return -1;
