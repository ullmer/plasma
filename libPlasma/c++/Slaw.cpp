...
//engaging with changes in C++11 and 17; generated by Gemini (Copilot struggled a bit here)
  detail::SlawIter it; // Declare 'it' here

#if __cplusplus >= 201103L // C++11 and later (covers C++17)
  it = remove_if(cmps.begin(), cmps.end(),
                 [&s](const detail::SlawRef& ref) { // CAPTURE 's' because this function uses 's'
                   return ref.Equals(s);
                 });
#else // Pre-C++11 fallback
  it = remove_if(cmps.begin(), cmps.end(),
                 bind2nd(mem_fun_ref(&detail::SlawRef::Equals), s));
#endif
